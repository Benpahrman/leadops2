...
from agents.domain.events import ProgressFeed, ProgressStatus
from agents.infra.logging import get_logger
...
class PortalService:
    ...
    def select_fields(self, slug: str, fields: list[str]) -> None:
        ...
        if self.storage:
            self.storage.save_sandbox(sandbox)
        try:
            from agents.services.client_artifacts import artifact_store
