...
class ContactEnricherResearcherAgent:
    ...
    def enrich_and_recover_lead(
        ...
        # Transition lead back to active pipeline (REVIEW)
        if lead.state == State.ARCHIVED:
            lead.transition(State.REVIEW, f"Contact email rehabilitated by ContactEnricherResearcherAgent ({new_email})")
        else:
            lead.state = State.REVIEW
        lead.consecutive_bounces = 0
        lead.log_event("CONTACT_ENRICHED", f"ContactEnricherResearcherAgent recovered lead with verified contact {new_email} ({new_name}, {new_role}).")
